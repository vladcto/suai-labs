...
theme: ПРОЕКТИРОВАНИЕ ИНТЕРФЕЙСА
variant: 5
number: 3
...
Изучить методы проектирования интерфейсов пользователей.
...
В лабораторной работе рассматривается создание информационной системы для доски мероприятий, которая предназначена для свободной публикации конференций, докладов, дебатов и т.п.  Целью работы является создание системы, которая позволит участникам сообщества свободно создавать и искать мероприятия.

## Краткие функциональные требования к ИС

Table: Виды пользователей

| Пользователь       | Описание задач                                                          |
|--------------------|-------------------------------------------------------------------------|
| Участник сообщества | Запись на конференции, просмотр информации о конференциях, редактирование конференций, удаление конференций |

## Основные форма приложения для работы с БД

Дизайн интерфейса проектировался с учетом его доступности в качестве web-cайта и нативного приложения на платформе `Android`. Для этого был выбран фреймворк `Flutter`

## Сценарии пользования и логика диалога с пользователем

### Сценарий 1: Вход и просмотр списка конференций

1. Вход в систему
   - Пользователь открывает приложение.
   - На экране логина вводит свои учетные данные (логин и пароль).
   - После успешной авторизации пользователь попадает на экран списка конференций.

2. Просмотр списка конференций
   - На экране пользователь видит список доступных конференций.
   - Для удобства поиска пользователь может использовать встроенные фильтры (по дате, теме или местоположению).

3. Выбор и просмотр конференции
   - Пользователь нажимает на интересующую его конференцию для просмотра детальной информации.

### Сценарий 2: Создание новой конференции

1. Доступ к функции создания конференции
   - На экране списка конференций пользователь нажимает кнопку добавления конференций.
   - Открывается экран создания конференции.

2. Заполнение формы создания конференции
   - Пользователь вводит необходимые детали конференции, такие как: название, описание, даты проведения, местоположение и тема.

3. Сохранение новой конференции
   - Пользователь нажимает кнопку «Сохранить».
   - Новая конференция добавляется в общий список.
   - Пользователь возвращается на экран списка конференций, где видит только что созданную конференцию.

### Сценарий 3: Редактирование и удаление конференции

1. Просмотр конференции для редактирования
   - В списке конференций пользователь выбирает конференцию, которую хочет изменить, и нажимает на ее название для просмотра детализированной информации.

2. Редактирование конференции
   - На экране подробной информации о конференции пользователь видит кнопку «Редактировать».
   - После нажатия кнопки открывается форма редактирования конференции.
   - Пользователь вносит изменения в необходимые поля.

3. Сохранение изменений
   - Пользователь нажимает кнопку «Сохранить изменения».
   - Измененная информация обновляется в системе и отобразится в общем списке.

4. Удаление конференции
   - На экране подробной информации о конференции пользователь нажимает кнопку «Удалить».
   - Появляется подтверждающее диалоговое окно.
   - После подтверждения конференция удаляется из списка, и пользователь возвращается на экран списка конференций.

## Пример использования

Для начала заходим в приложение и попадаем на экран логина.
...
В ходе выполнения лабораторной работы был разработан внешний интерфейс пользователя в статическом прототипировании с использованием `Flutter`, описаны краткие требования к информационной системе, а также описаны основные сценарии функций и диалог пользователя с системой.
