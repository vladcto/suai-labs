...
namespace TTT
{
    public class TerrainTools : EditorWindow
    {
        ...
        public bool additiveTrees = false; // adds new tree prototypes, instead of using existing ones
        ...
        public float grassclumping = 0.25f;
        ...
        static void DrawTitleGUI()
        {
            ...
            {
                Help.BrowseURL("https://web.archive.org/web/20200724223550/http://lemuria.org/Unity/TTT/");
            }
            GUILayout.EndHorizontal();
            EditorGUILayout.Separator();
        }
        ...
        void DrawTreeDistributionGUI()
        {
            ...
            if (toggleTrees)
            {
                ...
                GUILayout.EndHorizontal();
                GUILayout.BeginHorizontal();
                GUILayout.Label("Add as new trees (additive)");
                GUILayout.FlexibleSpace();
                additiveTrees = EditorGUILayout.Toggle("additive trees", additiveTrees);
                GUILayout.EndHorizontal();
                EditorGUILayout.Separator();
                GUILayout.BeginHorizontal();
        ...
        void AutoMagic()
        {
            ...
            if (treemap_name != null)
            {
                ...
                if (additiveTrees == true)
                {
                    Debug.LogWarning("additiveTrees was enabled, disabling it for automagic..");
                    additiveTrees = false; // cannot have this enabled for automagic
                }
                ApplyTreemap();
            }
        ...
        void AutoMagicDefaults()
        {
            ...
            switch (selectedTexturePack)
            {
                ...
                case 0:
                    tempFilePath = "Assets/Tom's Terrain Tools/Unity Terrain Assets/Textures/";
                    tempFileName = "Grass (Meadows)-png.png";
                    ...
                    tempFileName = "Grass (Hill)-png.png";
                    ...
                    tempFileName = "Grass&Rock-png.png";
                    ...
                    tempFileName = "Grass (Muddy)-png.png";
                ...
                case 1:
                    tempFilePath = "Assets/Tom's Terrain Tools/Unity Terrain Assets/Textures/SingleColor/";
                ...
                case 2:
                    tempFilePath = "Assets/Tom's Terrain Tools/Unity Terrain Assets/Textures/Debug/";
            ...
            // default trees
            tempFilePath = "Assets/Tom's Terrain Tools/Unity Terrain Assets/Trees Ambient-Occlusion/";
            tempFileName = "Alder.prefab";
            ...
            // default grass
            tempFilePath = "Assets/Tom's Terrain Tools/Unity Terrain Assets/Grass/";
            tempFileName = "Grass-tif.tif";
            ...
            tempFilePath = "Assets/Tom's Terrain Tools/Unity Terrain Assets/Bushes/";
            tempFileName = "Bush5.prefab";
            ...
            tempFilePath = "Assets/Tom's Terrain Tools/Unity Terrain Assets/Rocks/";
            tempFileName = "RockMesh.prefab";
            if (File.Exists(tempFilePath + tempFileName))
            {
        ...
        {
            if (myTerrain == null) myTerrain = Terrain.activeTerrain;
            if (myTerrain == null) { Debug.LogError("No terrain selected or no active terrain found!"); return; }

            myTerrain.terrainData.terrainLayers = SetupTextures();

            //Debug.Log("terrain name: "+myTerrain.name);
            string terrainAssetPath = AssetDatabase.GetAssetPath(myTerrain.terrainData);
            //Debug.Log("terrain asset path : "+terrainAssetPath);

            string terrainNameSafe = Path.GetFileNameWithoutExtension(terrainAssetPath);
            foreach (var c in Path.GetInvalidFileNameChars())
            {
                terrainNameSafe = terrainNameSafe.Replace(c, '_');
            }

            // get current terrains path
            string terrainPath = Path.GetDirectoryName(AssetDatabase.GetAssetPath(myTerrain.terrainData));

            // save layers to assets
            for (int i = 0; i < myTerrain.terrainData.terrainLayers.Length; i++)
            {
                string tempSavePath = Path.Combine(terrainPath, terrainNameSafe + "_layer" + i + ".asset");
                AssetDatabase.CreateAsset(myTerrain.terrainData.terrainLayers[i], tempSavePath);
            }
        }
        ...
        void ApplySplatmap()
        {
            ...
            {
                EditorUtility.DisplayDialog("ApplySplatmap - Missing Terrain Textures", "Please set up at least 4 textures in the Terrains own painter TerrainLayers list", "Cancel");
                return;
            }

            if (splatB != null && terrainData.alphamapLayers < 8)
            {
                EditorUtility.DisplayDialog("ApplySplatmap - Missing Terrain Textures", "Please set up at least 8 textures in the Terrains own painter TerrainLayers list", "Cancel");
                return;
            }
        ...
        void ApplyTreemap()
        {
            ...
            Undo.RecordObject(terrainData, "Apply tree map"); // warning, very slow!
            ...
            for (float y = 1; y < width - 1; y += Step)
            {
                ...
                for (float x = 1; x < width - 1; x += Step)
                {
                    ...
                    if (treeIndex >= 0)
                    {
                        ...
                        // for additive add to work, you need to manually place trees into terrain place trees list first!
                        if (additiveTrees == true)
                        {
                            treeInstance.prototypeIndex = treeIndex + terrainData.treePrototypes.Length - 3; // we add last 3 trees from the list
                        }
                        else
                        {
                            treeInstance.prototypeIndex = treeIndex; // tree index, from terrain place trees array
                        }
                        treeInstance.rotation = Random.value * (Mathf.PI * 2); // random rotation always enabled here
        ...
        void ApplyGrassmap()
        {
            ...
            Undo.RecordObject(terrainData, "Apply grass and bush maps"); // warning, very slow!
   ...
   //     }
        ...
        void AddTexture()
        {
            ...
            TerrainLayer[] oldLayers = myTerrain.terrainData.terrainLayers;
            TerrainLayer[] newLayers = new TerrainLayer[oldLayers.Length + 1];

            for (int x = 0; x < oldLayers.Length; x++)
            {
                newLayers[x] = oldLayers[x];
            }

            newLayers[oldLayers.Length] = new TerrainLayer();
            newLayers[oldLayers.Length].diffuseTexture = overlayTexture;
            newLayers[oldLayers.Length].tileSize = new Vector2(tileSize, tileSize);

            myTerrain.terrainData.terrainLayers = newLayers;
